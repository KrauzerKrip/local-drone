...
TaskMenu::TaskMenu(entt::registry* pRegistry, GuiDependencies dependencies) : Widget(dependencies) {
	...
	auto pBaseBackground = new ColorBackground(dependencies.pStyle->getColor("background_base"), dependencies);
	auto pDarkBackground = new ColorBackground(dependencies.pStyle->getColor("background_dark"), dependencies);
	// pDarkBackground->setStencil(Rectangle &rectangle)
	...
	m_pPlannedTaskList = new PlannedTaskList("Tasks", dependencies);
	...
	TaskList* pTaskList = m_pPlannedTaskList;
